<template>
    ...
    <div :style='{"margin":"0 10px 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}' class="select" label="宠物类型" prop="chongwuleixing">
        ...
        <el-select clearable v-model="searchForm.chongwuleixing" placeholder="请选择宠物类型" >
            <el-option v-for="(item,index) in chongwuleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
        </el-select>
    </div>
		<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}' class="select">
						...
						<el-select clearable v-model="searchForm.sfsh" placeholder="是否审核">
    ...
    <el-button class="search" type="success" @click="search()">
        <span class="icon iconfont icon-chakan14" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"block","height":"40px"}'></span>
				...
				<el-row class="actions" :style='{"padding":"0px","margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","background":"none","display":"flex","width":"auto","fontSize":"12px"}'>
					...
					<el-button class="btn18" v-if="isAuth('chongwulingyang','宠物信息类型占比')" type="success" @click="chartDialog1()">
...
<el-table-column :resizable='true' :sortable='true' prop="fengmian" width="200" label="宠物照片">
						<template slot-scope="scope">
...
<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('chongwulingyang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="chongwuleixingChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var chongwuleixingChart1 = echarts.init(document.getElementById("chongwuleixingChart1"),'macarons');
        this.$http({
            url: "chongwulingyang/group/chongwuleixing",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                chongwuleixingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chongwuleixingChart1.resize();
                };
            }
        });
      })
    ...
    shBatchDialog(){
		for(let x in this.dataListSelections){
			if(this.dataListSelections[x].sfsh&&this.dataListSelections[x].sfsh!='待审核'){
				this.$message.error('存在已审核数据，不能审核');
				this.batchIds = []
				return false
			}
	...
	chongwulingyangstatusChange(e,row){
		if(row.status==0){
