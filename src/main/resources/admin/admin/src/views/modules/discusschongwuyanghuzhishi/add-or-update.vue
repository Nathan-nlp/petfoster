...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusschongwuyanghuzhishi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `discusschongwuyanghuzhishi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discusschongwuyanghuzhishiCrossAddOrUpdateFlag = false;
											this.parent.search();
