...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 6px","margin":"172px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"right","justifyContent":"center","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"380px","borderStyle":"solid","height":"auto","order":"1"}'>
	  ...
	  <el-button v-if="btnAuth('chongwuyanghuzhishi','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"100px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/chongwuyanghuzhishiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
    ...
    <div :style='{"width":"100%","padding":"20px","background":"#f5f5f5","display":"flex","flexWrap":"wrap","justifyContent":"space-between"}' class="list list4 index-pv1">
        <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"32%","margin":"0 0 20px 0","overflow":"hidden","position":"relative","background":"#fff","borderRadius":"8px","boxShadow":"0 2px 12px 0 rgba(0,0,0,0.1)","height":"480px","transition":"all 0.3s"}' class="item" @mouseover="hoverItem(index)" @mouseout="hoverItem(null)">
            <img @click.stop="imgPreView(item.tupian)" v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian.split(',')[0]" :style='{"width":"100%","height":"240px","objectFit":"cover","borderRadius":"8px 8px 0 0"}' />
            <img @click.stop="imgPreView(baseUrl + (item.tupian?item.tupian.split(',')[0]:''))" v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" :style='{"width":"100%","height":"240px","objectFit":"cover","borderRadius":"8px 8px 0 0"}' />
            
            <div :style='{"padding":"15px","display":"flex","flexDirection":"column","justifyContent":"space-between","height":"240px"}'>
                <div :style='{"marginBottom":"15px"}'>
                    <div :style='{"fontSize":"18px","fontWeight":"bold","color":"#333","marginBottom":"10px","overflow":"hidden","textOverflow":"ellipsis","whiteSpace":"nowrap"}'>{{item.biaoti}}</div>
                    
                    <div :style='{"display":"flex","flexWrap":"wrap"}'>
                        <div :style='{"width":"50%","marginBottom":"8px","fontSize":"14px","color":"#666"}'>
                            <span :style='{"color":"#999","marginRight":"5px"}'></span>{{item.yanghufenlei}}
                        </div>
                        <div :style='{"width":"50%","marginBottom":"8px","fontSize":"14px","color":"#666"}'>
                            <span :style='{"color":"#999","marginRight":"5px"}'>适用于:</span>{{item.shiyong}}
                        </div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/yanghufenlei/yanghufenlei').then(res => {
			if (res.data.code == 0) {
				this.yanghufenleiOptions = res.data.data;
				this.selectOptionsList.push({name:'养护分类',list:this.yanghufenleiOptions,tableName: 'yanghufenlei',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('yanghufenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'yanghufenlei'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.yanghufenlei != '') searchWhere.yanghufenlei = this.formSearch.yanghufenlei;
        if (this.curFenlei != '全部') searchWhere.yanghufenlei = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`chongwuyanghuzhishi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/chongwuyanghuzhishiDetail', query: params});
      },
